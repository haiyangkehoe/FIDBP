...
plot_idbp = 1; %=1 to plot IDBP movie in MATLAB
save_idbp = 1; %=1 to save IDBP textfiles
save_var = 1; %=1 to save MATLAB variables

%User inputs 
ms_path='Aftershock_0_xcor/';
as_path='Aftershock_0_syn_xcor/';
as_pad_path=strcat('Pad_',as_path);
idbp_path=strcat('IDBP_',ms_path(1:end-1),'_by_',as_path);
cont = 0.30; %Mainshock back-projection contour above which grid points are inverted
...
ms_ts = 0.5; %Mainshock back-projection data timestep
as_ts = 0.5; %Afterhock back-projection data timestep
...
%ms_hyp_cont = 0.9; %Contour to estimate mainshock hypocenter 
%as_hyp_cont = 0.9; %Contour to estimate aftershock hypocenter 
